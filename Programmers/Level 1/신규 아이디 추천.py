...
def solution(new_id):
    ...
    # .를 기준으로 문자열을 나눠 저장한 뒤, .반복 여부 확인
    p = re.compile('[.]*[a-z0-9-_]*')
    temp = ''
    for x in p.findall(answer):
        # .이 1개 이상인 경우, 모든 .를 제거한 뒤 '.' + x를 temp에 추가
        if x.count('.') > 1:
            temp += '.' + x.replace('.', '')
            continue
        temp += x
    ...
    # 시작과 끝을 참조하기 때문에 deque로 선언
    # deque를 사용하지 않고, 6번과 같이 해결 가능
    temp = deque(answer)
    if temp and temp[0] == '.': temp.popleft()
