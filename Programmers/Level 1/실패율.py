...
def solution(N, stages):
    ...
    # 스테이지 정보 저장
    # 하위 라운드 클리어 정보 저장 로직을 위해 역순으로 변경
    # 최종 스테이지를 깬 경우를 고려해 범위 1 증가
    for x in range(N + 1)[::-1]:
        stage_clear_dict[x + 1] = [0, 0]
        
    # 현재 스테이지 값에서 -1하여 클리어한 정보 저장
    # 스테이지 도달 인원 저장 로직 추가
    for x in stages:
        if x > 1: stage_clear_dict[x - 1][0] += 1
        if x >= 1:stage_clear_dict[x][1] += 1
        
    # 상위 스테이지를 클리어한 유저의 데이터를 누적하여 스테이지 별 클리어 정도 갱신
    # 스테이지 도달 인원 갱신 로직 추가
    for x in range(N)[::-1]:
        clear_cnt += stage_clear_dict[x + 1][0]
        stage_clear_dict[x + 1][0] = clear_cnt
        stage_clear_dict[x + 1][1] += clear_cnt
